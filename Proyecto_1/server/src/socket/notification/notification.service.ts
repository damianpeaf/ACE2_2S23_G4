...
export class NotificationService {
  ...
  public globalState: GlobalStateI = {
    is_light_on: false,
    vent_state: 'off',
    presence: false,
  };
  ...
  setGlobalState(incomingState: Partial<GlobalStateI>) {
    if (
      incomingState.is_light_on !== undefined &&
      incomingState.is_light_on !== this.globalState.is_light_on
    ) {
      this.globalState.is_light_on = incomingState.is_light_on;
    }

    if (
      incomingState.vent_state !== undefined &&
      incomingState.vent_state !== this.globalState.vent_state
    ) {
      this.globalState.vent_state = incomingState.vent_state;
    }

    if (
      incomingState.presence !== undefined &&
      incomingState.presence !== this.globalState.presence
    ) {
  ...
  analyzeData(event: LiveDataEvent) {
    const { air_quality, presence, timestamp, light, temperature } =
      event.payload;

    // analyze light
    this.analyzeLight(timestamp, air_quality, presence, light, temperature);
    // analyze air quality
    this.analyzeAirQuality(
      timestamp,
      air_quality,
      presence,
      light,
        ...
        if (result) {
          ...
          if (
            ...
            const notification: NotificationI = {
              message: `La luz se ha quedado encendida sin presencia humana por más de 25 segundos. Temperatura: ${temperature}°C, Calidad del aire: ${air_quality} ppm, Luz: ${light} lux, Presencia: ${
                presence ? 'Si' : 'No'
              }`,
            ...
            const notification: NotificationI = {
              message: `La luz se ha quedado encendida sin presencia humana por más de 50 segundos, se apagará automáticamente. Temperatura: ${temperature}°C, Calidad del aire: ${air_quality} ppm, Luz: ${light} lux, Presencia: ${
                presence ? 'Si' : 'No'
              }`,
              timestamp: new Date().toISOString(),
              type: 'error',
  ...
  async analyzeAirQuality(
    ...
    if (this.notificationState.secondAirNotification) {
      ...
      const notification: NotificationI = {
        message: `La calidad del aire se ha restablecido. Temperatura: ${temperature}°C, Calidad del aire: ${air_quality} ppm, Luz: ${light} lux, Presencia: ${
          presence ? 'Si' : 'No'
        }`,
        timestamp: new Date().toISOString(),
        type: 'info',
      ...
      this.redisService.get('airNotification', (err, result) => {
        ...
        if (result) {
          ...
          if (
            ...
            const notification: NotificationI = {
              message: `La calidad del aire es mala de manera sostendida por más de 10 segundos. Temperatura: ${temperature}°C, Calidad del aire: ${air_quality} ppm, Luz: ${light} lux, Presencia: ${
                presence ? 'Si' : 'No'
              }`,
              timestamp: new Date().toISOString(),
              type: 'warning',
            ...
            const notification: NotificationI = {
              message: `La calidad del aire es mala, de manera sostendida por más de 20 segundos, se prenderá el ventilador automáticamente. Temperatura: ${temperature}°C, Calidad del aire: ${air_quality} ppm, Luz: ${light} lux, Presencia: ${
                presence ? 'Si' : 'No'
              }`,
              timestamp: new Date().toISOString(),
              type: 'error',
